...
from sleeper_client import get_projections_nfl


def normalize_projection(entry: dict) -> dict:
    player_id = str(entry.get("player_id") or entry.get("player"))
    stats = entry.get("stats") or {}
    # Sleeper provides fantasy points in various fields; prefer 'pts_ppr' if present
    points = stats.get("pts_ppr") or stats.get("pts_half_ppr") or stats.get("pts_std") or entry.get("fantasy_points") or 0
...
def main(argv: List[str] | None = None) -> int:
    ...
    for pos in ["QB", "RB", "WR", "TE", "K", "DEF"]:
        try:
            data = get_projections_nfl(season, week, pos)
        except Exception:
            data = []
        for e in data:
